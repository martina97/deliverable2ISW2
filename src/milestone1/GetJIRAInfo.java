...
public class GetJIRAInfo {
	  ...
	  public static List<Release> getListRelease() throws IOException, JSONException {
	   ...
	   public static List<Ticket> compareDateVersion(List<Release> releases2, List<LocalDateTime> dateList, List<Ticket> listaTicket) {
		
		   
		   //releases è una lista di release che hanno data e versione (dal codice Falessi)
	   ...
	   public static void printArrayList(List<Ticket> listaTicket) {
			for (int i = 0; i < listaTicket.size(); i++) {
				...
				Log.infoLog(listaTicket.get(i).getIndex() + ")\t\tticket = " + listaTicket.get(i).getID() + "\t\tcreated: " + listaTicket.get(i).getCreationDate() +
						"\t\tresolution: " + listaTicket.get(i).getResolutionDate() + "\t\tIV = " + + listaTicket.get(i).getIV() + "\t\t OV: "+ listaTicket.get(i).getOV() 
						+"\t\tFV: " + listaTicket.get(i).getFV() +"\t\tAV = " +listaTicket.get(i).getAV());
			}
		}
	   
	   
	   
	   //scorro la lista dei ticket, e ai ticket con release date o creation date assegno le versioni trovate
	   ...
	   public static List<Ticket> retrieveTickets(String projName, List<Release> releases) throws IOException, JSONException {
	    ...
	    do {
	       ...
	       for (; i < total && i < j; i++) { //for sulla issue
	          ...
	          //String versions = issues.getJSONObject(i%1000).get("versions").toString();	//PRINT KEY DELLA ISSUE
	    	...
	    	//CharSequence resolutiondate1 = fields.getString("resolutiondate").subSequence(0,19);	//FV
	    	
	    	 
	    	
	    	
	    	LocalDateTime resolutiondate = LocalDateTime.parse(fields.getString("resolutiondate").subSequence(0,19));
	   ...
	   public static List<LocalDateTime> addAndSortDateTickets(List<Ticket> listaTicket) {
	 		...
	 		//ho lasciayo localdatetime, così ho anche l'orario di FV e OV, anche se avrei potuto mettere localdate ma 
	 		//avrei dovuto cambiare il codice di falessi
	 		
	 		//cosi facendo non ci sono duplicati (è difficile trovare orario uguale di FV e Ov dello stesso giorno)
